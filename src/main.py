...
class AuthDialog:
        ...
        self.dialog = ft.AlertDialog(
            ...
            content=ft.Column([
                ft.TextField(
                    ...
                    hint_text="Введите свой OPENROUTER_API_KEY",
                    width=300,
                    on_submit=self.check_credentials
                ),
                ft.TextField(
                    label="PIN пароль",
                    hint_text="Введите свой PIN пароль",
                    width=300,
                    password=True,
                    on_submit=self.check_credentials
                )
            ...
            actions=[
                ...
                ft.TextButton("Войти", on_click=lambda e: self.check_credentials(e)),
                ft.TextButton("Сбросить PIN", on_click=lambda e: self.reset_pin(e))
            ],
        )
    ...
    async def update_api_key(self, api_key):
        ...
        # Изменение значения OPENROUTER_API_KEY
        ...
        self.app.OPENROUTER_API_KEY = api_key
        self.app.api_client = OpenRouterClient()
        self.app.update_balance()

    def check_credentials(self, e):
        api_key = self.dialog.content.controls[0].value
        pin = self.dialog.content.controls[1].value
        ...
        if not os.path.exists(pin_file):
            ...
            if pin == saved_pin:
                self.close_dialog()
                asyncio.run(self.update_api_key(api_key))
                self.app.main(self.page)  # Запуск основного приложения
            else:
                self.dialog.content.controls[0].error_text = "Неправильный PIN пароль"
                self.dialog.update()

    def create_pin(self):
        self.dialog.content.controls[0].label = "OPENROUTER_API_KEY"
        self.dialog.content.controls[0].hint_text = "Введите свой OPENROUTER_API_KEY"
        self.dialog.content.controls[1].label = "Создайте PIN пароль"
        self.dialog.content.controls[1].hint_text = "Введите свой PIN пароль"
        self.dialog.actions[1].text = "Создать"
        self.dialog.update()
        self.dialog.content.controls[1].on_submit = self.save_pin

    def save_pin(self, e):
        pin = self.dialog.content.controls[1].value
        with open("pin.txt", "w") as f:
            f.write(pin)
        self.dialog.content.controls[1].label = "PIN пароль"
        self.dialog.content.controls[1].hint_text = "Введите свой PIN пароль"
        self.dialog.actions[1].text = "Войти"
        self.dialog.update()
        self.dialog.content.controls[1].on_submit = self.check_credentials

    def reset_pin(self, e=None):
        os.remove("pin.txt")
        self.dialog.content.controls[1].label = "Создайте PIN пароль"
        self.dialog.content.controls[1].hint_text = "Введите свой PIN пароль"
        self.dialog.actions[1].text = "Создать"
...
def main(page: ft.Page):
    ...
    app = ChatApp()
    auth_dialog = AuthDialog(app, page)
    page.overlay.append(auth_dialog.dialog)
    auth_dialog.dialog.open = True
    page.update()
